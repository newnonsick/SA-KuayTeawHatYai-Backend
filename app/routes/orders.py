...
@orders_blueprint.route('/orders/update-table', methods=['PUT'])
def update_order():
    data = request.get_json()
    order_id = data.get("order_id")
    table_number = data.get("table_number")

    if not all([order_id, table_number]):
        raise ValueError("Missing required fields.")
    ...
    query = "UPDATE ORDERS SET table_number = %s WHERE order_id = %s"
    execute_command(query, (table_number, order_id))
