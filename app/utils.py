...
def generate_token(time_previous = None) -> str:
    ...
    timestamp = int(time_previous) if time_previous else int(time.time() // 10)
...
def verify_token(token: str) -> bool:
    """ตรวจสอบว่า Token ถูกต้องหรือไม่ (รองรับการคลาดเคลื่อน 10 วินาที)"""
    current_token = generate_token()
    previous_token = generate_token(int(time.time() // 10) - 1)
